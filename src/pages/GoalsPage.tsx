...
const EmptyState: React.FC<{ onCreate: () => void }> = ({ onCreate }) => (
  <div className="border border-white/10 rounded-2xl p-8 text-center bg-white/5">
    ...
    <button onClick={onCreate} className="px-4 py-2 rounded-xl bg-white text-gray-900 font-semibold">
      + New Goal
    </button>
  </div>
)
...
const GoalRow: React.FC<GoalRowProps> = ({ goal, isOpen, onToggle, onAddBucket, onAddSuggestion, onToggleBucketFavorite }) => {
  ...
  return (
    <div className="rounded-2xl bg-white/5 hover:bg-white/10 transition border border-white/5">
      ...
      {isOpen && (
        <div className="relative px-4 md:px-5 pb-4 md:pb-5 overflow-hidden">
          ...
          <ul className="mt-3 space-y-2">
            {goal.buckets.length === 0 && (
              <li className="text-white/60 text-sm">No buckets yet. Add 2–4 to keep it focused.</li>
            )}
            {goal.buckets.map((b) => (
    ...
    <div className="goals-layer min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-white antialiased">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <section className="mb-6">
          ...
          <p className="text-white/70 mt-1">Sleek rows with thin gradient progress. Tap to see Task Bank. Add buckets and quick suggestions inside.</p>
        </section>
        ...
        <div className="fixed inset-0 z-30 grid place-items-center bg-black/60 p-6">
          <div className="w-full max-w-lg rounded-2xl bg-slate-900 border border-white/10 p-6 shadow-2xl">
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button onClick={() => setShowNew(false)} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20">
                Cancel
              </button>
              <button onClick={addGoal} className="px-4 py-2 rounded-xl bg-white text-gray-900 font-semibold">
                Create
              </button>
            </div>
          </div>
